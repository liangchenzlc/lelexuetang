...
import com.lelexuetang.base.utils.IdWorkerUtils;
import com.lelexuetang.base.utils.QRCodeUtil;
...
public class OrderServiceImpl implements OrderService {
    ...
    public PayRecordDto createOrder(String userId, AddOrderDto addOrderDto) {
        ...
        try {
            ...
            LeLeXueTangException.cast("生成二维码出错");
        }
    ...
    public PayStatusDto queryPayResultFromAlipay(String payNo){
        ...
        try {
            ...
            if(!response.isSuccess()){//交易不成功
                LeLeXueTangException.cast("请求支付宝查询支付结果失败");
            }
            body = response.getBody();
        } catch (AlipayApiException e) {
            e.printStackTrace();
           LeLeXueTangException.cast("请求支付查询支付结果异常");
        }
    ...
    public void saveAliPayStatus(PayStatusDto payStatusDto){
        ...
        if(payRecordByPayno == null){
            LeLeXueTangException.cast("找不到相关的支付记录");
        }
        //拿到相关联的订单id
        Long orderId = payRecordByPayno.getOrderId();
        XcOrders xcOrders = ordersMapper.selectById(orderId);
        if(xcOrders == null){
            LeLeXueTangException.cast("找不到相关联的订单");
        }
        //支付状态
    ...
    public XcPayRecord createPayRecord(XcOrders orders){
        ...
        if(insert<=0){
            LeLeXueTangException.cast("插入支付记录失败");
        }
        return xcPayRecord;
    }
    ...
    public XcOrders saveXcOrders(String userId, AddOrderDto addOrderDto){
        ...
        if(insert<=0){
            LeLeXueTangException.cast("添加订单失败");
        }
        //订单id
